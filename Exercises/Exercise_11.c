...
1.int* total_number_of_books;

 * This stores the total number of books on each shelf.

2.int** total_number_of_pages;
...
int main()
{
    ...
    while (total_number_of_queries--) {
        ...
        if (type_of_query == 1) {
            ...
            // Increase the total number of books on shelf x by 1
            (*(total_number_of_books + x))++;
            
            // Reallocate memory for the books on shelf x
            total_number_of_pages[x] = realloc(total_number_of_pages[x], (*(total_number_of_books + x)) * sizeof(int));
            
            // Add the number of pages for the new book at the end of shelf x
